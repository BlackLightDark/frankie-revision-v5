...
let revisionUpcomingOnly = false;
...
function getCurrentWeekScore() {
  ...
  const currentWeek = weeks.find(week =>
    week.days.some(d => parseDate(d.date).getTime() === today.getTime())
  ) || weeks.find(week =>
    week.days.some(d => parseDate(d.date) >= today)
  ...
  currentWeek.days.forEach((day, di) => {
    day.tasks.forEach((task, ti) => {
      total++;
      const wi = weeks.indexOf(currentWeek);
...
function renderCountdownStrip() {
  ...
  if (!container) return;
  const today = new Date(); today.setHours(0,0,0,0);
  const upcoming = examTimetable
    .filter(e => !e.coursework)
    .filter(e => parseExamDate(e.date) >= today)
  ...
  container.innerHTML = upcoming.map(exam => {
    ...
    const big = days === 0 ? 'Today' : days === 1 ? '1 day' : `${days} days`;
    return `
      <div class="countdown-tile">
        <div class="countdown-kicker">Coming up</div>
        <div class="countdown-big">${big}</div>
        <div class="countdown-subject">${exam.subject}</div>
        <div class="countdown-paper">${exam.paper.replace(/\(.*?\)/g,'').trim()} · ${exam.date} 2026</div>
      </div>
    `;
  }).join('');
}
...
function renderDashboard() {
  ...
  const weekScoreCopy = document.getElementById('week-score-copy');
  ...
  const currentWeek = getCurrentWeekScore();
  if (weekScore) weekScore.textContent = currentWeek.pct + '%';
  if (weekScoreCopy) weekScoreCopy.textContent = `${currentWeek.done} of ${currentWeek.total} tasks done in ${currentWeek.week.label}.`;
...
function switchTab(name) {
  ...
  if (name === 'examtable') renderExamTable();
}
...
const subjects = [
  {
    ...
    cards: [
      ...
      { q: 'Maths Paper 1 non-calculator: what should you do differently?', a: 'Lean on algebra, fractions, exact values, estimation and clean working. Do not panic and turn it into a guessing contest — method marks matter.' },
      { q: 'How do you rearrange a formula safely?', a: 'Undo operations in reverse order, keep both sides balanced, and watch brackets like a hawk. If the variable appears twice, factorise it out.' },
      { q: 'What should you check on calculator papers?', a: 'Mode (degrees!), negative signs, brackets, rounding, and whether the answer is sensible. A calculator is useful, but it is also fully capable of betraying you.' },
      { q: 'What is the best approach to histograms and cumulative frequency?', a: 'Histogram uses frequency density = frequency ÷ class width. Cumulative frequency graphs help estimate median, quartiles and interquartile range.' },
      { q: 'What does a good final maths check look like?', a: 'Re-read the command word, check units, check rounding, and substitute back where possible. Ten seconds of checking can rescue a ridiculous answer.' },
    ]
  },
  {
    ...
    cards: [
      ...
      { q: 'English Lit Paper 1: what is the smart structure for Macbeth?', a: 'Short argument-led intro, 3–4 analytical paragraphs with quotations, then a sharp conclusion. Stay on the question — no waffle safari.' },
      { q: 'How should you revise Macbeth quotes efficiently?', a: 'Learn short flexible quotes tied to big themes: ambition, guilt, kingship, supernatural, masculinity. A few sharp quotes beat a suitcase full of half-remembered ones.' },
      { q: 'English Lit Paper 2: what must you remember for poetry comparison?', a: 'Compare throughout, not poem-then-poem. Link ideas, methods and effects. Keep returning to the question and the big theme.' },
      { q: 'How do you tackle unseen poetry without spiralling?', a: 'Read once for gist, again for methods, then zoom in on a few strong choices. Meaning first, methods second — not the other way round.' },
      { q: 'What makes a strong literature paragraph?', a: 'Point, embedded quotation, analysis of method, effect, and link back to the question. Context only when it genuinely earns its keep.' },
    ]
  },
  {
    ...
    cards: [
      ...
      { q: 'English Language Paper 1: what is the order of attack?', a: 'Q1 quick retrieval, Q2 language, Q3 structure, Q4 evaluation, then Q5 writing. Keep moving — do not let one awkward question eat the paper.' },
      { q: 'How do you avoid weak language analysis?', a: 'Do not just name a technique. Zoom in on key words, explain connotations, and say what the reader is meant to think or feel.' },
      { q: 'What should you do in Question 5 narrative writing?', a: 'Keep the plot manageable, control the viewpoint, and write vividly. One strong scene beats a chaotic mini Netflix series.' },
      { q: 'English Language Paper 2: what matters most?', a: 'Clear comparison, writer methods, viewpoints and tone. Keep using both sources rather than forgetting one halfway through.' },
      { q: 'How do you pick evidence fast in language exams?', a: 'Underline as you read, choose short quotations, and avoid dragging in huge chunks. Small precise evidence is easier to analyse well.' },
    ]
  },
  {
    ...
    cards: [
      ...
      { q: 'Biology Paper 1: what topics usually dominate?', a: 'Cell biology, organisation, infection and response, bioenergetics, and often practical skills. Know the required practicals, not just the pretty bits.' },
      { q: 'Biology Paper 2: what should you expect?', a: 'Homeostasis, inheritance, variation and evolution, ecology, plus data and application. The wording often matters as much as the science.' },
      { q: 'How do you revise required practicals for Biology?', a: 'Know method, variables, controls, risks, and why each step matters. Examiners love practical questions because panic is apparently a hobby.' },
      { q: 'What is the best way to answer six-mark biology questions?', a: 'Write in logical sequence, use precise terms, and make each sentence add something new. Think process, not random fact-dumping.' },
      { q: 'What is a common mistake in biology exams?', a: 'Giving vague everyday answers instead of exact science vocabulary. If you know the term, use it.' },
    ]
  },
  {
    ...
    cards: [
      ...
      { q: 'Chemistry Paper 1: what should be rock solid?', a: 'Atomic structure, bonding, quantitative chemistry, chemical changes, and energy changes. Equations and ion charges need to be automatic.' },
      { q: 'Chemistry Paper 2: what usually needs extra care?', a: 'Rates, organic chemistry, chemical analysis, atmosphere and resources. Read every question carefully because one word can change the whole answer.' },
      { q: 'How do you survive chemistry calculations?', a: 'Write the formula first, substitute values carefully, carry units, and only round at the end. Random number wizardry is not a method.' },
      { q: 'What should you know for chemical tests?', a: 'Flame tests, gas tests, ions in solution, and why the result identifies the substance. Memorise the patterns properly.' },
      { q: 'What is the golden rule for balancing equations?', a: 'Change coefficients only, never the little numbers in formulas. Otherwise you are inventing new chemistry, which the exam board tends to dislike.' },
    ]
  },
  {
    ...
    cards: [
      ...
      { q: 'Physics Paper 1: what should feel secure?', a: 'Energy, electricity, particle model and atomic structure. Equations, units and circuit ideas need to be fast and clean.' },
      { q: 'Physics Paper 2: what catches people out?', a: 'Forces, waves, magnetism and electromagnetism. Diagrams, direction, and command words matter a lot.' },
      { q: 'How do you improve in physics fast?', a: 'Practise equation questions, rearrange formulas confidently, and explain processes step by step. Physics rewards calm working more than heroic vibes.' },
      { q: 'What should you always check in physics answers?', a: 'Units, prefixes, powers of ten, and whether the answer makes physical sense. A bus travelling at light speed is usually a clue something went wrong.' },
      { q: 'How do you handle required practical questions in physics?', a: 'State the method clearly, name control variables, mention repeat readings, and explain how accuracy is improved.' },
    ]
  },
  {
    ...
    cards: [
      ...
      { q: 'History: what makes a strong exam answer under pressure?', a: 'Specific knowledge, direct explanation, and constant focus on the question. Dates and names are useful only if they actually prove something.' },
      { q: 'How should you revise the Migrants in Britain paper?', a: 'Learn key groups, reasons for migration, treatment on arrival, and impact over time. Build mini timelines so the story sticks.' },
      { q: 'How do you revise Elizabethan England efficiently?', a: 'Focus on religion, plots, Mary Queen of Scots, Armada, society and exploration. Sort material into causes, events and significance.' },
      { q: 'What matters most for the USA / Middle East units?', a: 'Key turning points, leaders, causes of tension, and consequences. Think chronology first, detail second.' },
      { q: 'How do you avoid weak history paragraphs?', a: 'Do not just narrate events. Make a point, support it with evidence, and explain why it matters.' },
    ]
  },
  {
    ...
    cards: [
      ...
      { q: 'Geography Paper 1: what should you know cold?', a: 'Natural hazards, ecosystems and either coasts or rivers depending on the course. Case studies need sharp facts, not foggy vibes.' },
      { q: 'Geography Paper 2: what is the priority?', a: 'Urban issues, development, and resource management themes. Learn named examples and be ready to compare places clearly.' },
      { q: 'Geography Paper 3: what do examiners want?', a: 'Application of fieldwork, issue evaluation, and decision-making. Use evidence, justify choices, and stay practical.' },
      { q: 'How should you revise case studies?', a: 'Use tiny fact packs: place, causes, effects, responses, and one or two statistics. Enough to sound convincing without memorising a textbook brick.' },
      { q: 'What makes a strong 6- or 9-mark geography answer?', a: 'Balanced points, place-specific evidence, and a judgement when needed. Generic answers are the enemy.' },
    ]
  },
  {
    ...
    cards: [
      ...
      { q: 'Art exam prep: what should the portfolio prove?', a: 'That ideas developed over time, artists informed choices, experimentation happened, and the final outcome was personal and deliberate.' },
      { q: 'What should you do before the externally set task?', a: 'Sort references, refine experiments, plan composition options, and know exactly what materials you will use. Remove avoidable chaos.' },
      { q: 'How can annotation earn marks instead of just existing?', a: 'Be specific about decisions, influences, materials, problems and next steps. Real reflection beats decorative filler every time.' },
      { q: 'What is the danger in GCSE Art exam work?', a: 'Playing safe and repeating old ideas without development. The work should look connected to your theme, not copied from autopilot.' },
    ]
  },
  {
    ...
    cards: [
      ...
      { q: 'Music appraising exam: what should you do first when listening?', a: 'Lock onto pulse, texture, instruments and structure before chasing tiny details. Big picture first, then fine detail.' },
      { q: 'How should you revise set works?', a: 'Know key features, context, instrumentation, and a few precise musical moments. Listen actively rather than just having it on in the background.' },
      { q: 'What helps in unfamiliar listening questions?', a: 'Use MADTSHIRT and write what you genuinely hear with proper vocabulary. Precise simple points score better than vague fancy nonsense.' },
      { q: 'How do performance and composition link back to revision?', a: 'Understanding structure, harmony, rhythm and texture in your own work makes it easier to spot them in the exam too.' },
      { q: 'What is the easiest way to lose marks in music appraising?', a: 'Being too generic: saying it is fast or loud without naming tempo, dynamics, texture or instruments properly.' },
    ]
  },
];
...
function renderSubjectGrid() {
  ...
  if (!grid) return;
  const filteredSubjects = getRevisionSubjects();
  const upcomingMap = getUpcomingExamSubjectMap();
  grid.innerHTML = '';

  filteredSubjects.forEach(s => {
    const nextInfo = upcomingMap[s.id];
    const card = document.createElement('div');
    card.className = 'subject-card';
    const pill = nextInfo ? `<span class="next-pill">${nextInfo.daysLabel}</span>` : '';
    const meta = nextInfo
      ? `<div class="subject-meta">Next exam: ${nextInfo.exam.subject} · ${nextInfo.exam.date} 2026</div>`
      : `<div class="subject-meta">${s.cards.length} flashcards ready</div>`;
  ...
  const copy = document.getElementById('revision-filter-copy');
  const btn = document.getElementById('revision-upcoming-btn');
  if (btn) btn.classList.toggle('active', revisionUpcomingOnly);

  const subjectCount = filteredSubjects.length;
  if (copy) {
    if (revisionUpcomingOnly) {
      const names = filteredSubjects.map(s => s.label).join(', ');
      copy.textContent = filteredSubjects.length
        ? `Showing ${subjectCount} subject${subjectCount === 1 ? '' : 's'} with exams still ahead.`
        : 'No upcoming exam subjects found in the remaining timetable. Very suspicious.';
    } else {
      copy.textContent = 'Showing the full deck.';
    }
  }
}
...
function subjectIdFromExamSubject(subjectName) {
  const map = {
    'English Lit': 'lit',
...
function getUpcomingExamSubjectMap() {
  const today = new Date(); today.setHours(0,0,0,0);
  const map = {};
  examTimetable
    .filter(e => !e.coursework)
    ...
    .sort((a,b) => parseExamDate(a.date) - parseExamDate(b.date))
    .forEach(exam => {
      const id = subjectIdFromExamSubject(exam.subject);
      if (!id || map[id]) return;
      const examDate = parseExamDate(exam.date);
      const days = Math.round((examDate - today) / 86400000);
      map[id] = {
...
function getRevisionSubjects() {
  const upcomingMap = getUpcomingExamSubjectMap();
  return revisionUpcomingOnly
    ? subjects.filter(s => upcomingMap[s.id])
    : subjects;
}

function toggleUpcomingRevisionMode() {
  revisionUpcomingOnly = !revisionUpcomingOnly;
  renderSubjectGrid();
}
...
function renderExamTable() {
  ...
  // Re-query dashboard nodes here instead of relying on accidental global IDs.
  // The previous version could throw a ReferenceError here, which then tripped the
  // init catch block and made page 1 look like it had failed to load.
  const weekScore = document.getElementById('week-score');
  const weekScoreCopy = document.getElementById('week-score-copy');
...
function initFrankiePlanner() {
  try {
    ...
    console.error('Frankie planner failed to initialise', err);
    const weeksContainer = document.getElementById('weeks-container');
    if (weeksContainer) {
      weeksContainer.innerHTML = '<div style="background:white;border:1.5px solid var(--border);border-radius:10px;padding:16px;color:var(--ink-light);">Planner hit a loading wobble. The JavaScript tripped while loading. This v6 build should fix that — if you still see this, hard refresh the page.</div>';
    }
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initFrankiePlanner);
} else {
  initFrankiePlanner();
}
